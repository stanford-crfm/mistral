...
sys.path.insert(0, '..')
from utils.custom_modeling_gpt2 import GPT2ForTokenClassification
...
class DataTrainingArguments:
    ...
    return_macro_metrics: bool = field(
        default=False,
        metadata={"help": "Whether to return macro_metrics during evaluation or just the overall ones (micro)."},
    )
    return_avg_metric_across_entities: bool = field(default=False)
...
def main():
    ...
    if config.model_type == "gpt2":
        model_class = GPT2ForTokenClassification
    else:
    ...
    def compute_metrics(p):
        ...
        if data_args.return_avg_metric_across_entities:
            results = metric.compute(predictions=[sum(true_predictions, [])], references=[sum(true_labels, [])])
            print ('\nreturn_avg_metric_across_entities\n')
        else:
