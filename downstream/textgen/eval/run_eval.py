...
def run_evaluate(verbose=True):
    ...
    output_lns = [clean_text(x.rstrip()) for x in open(args.save_path).readlines()]
    reference_lns = [x.rstrip() for x in open(args.reference_path).readlines()][: len(output_lns)]
    if args.num_examples != -1:
        output_lns = output_lns[:args.num_examples]
        reference_lns = reference_lns[:args.num_examples]
    else:
