...
if __name__ == '__main__':
    ...
    parser.add_argument('--preseqlen', type=int, default=0, help='')
    ...
    parser.add_argument('--model_nickname', type=str, default='gpt2', help='')
    ...
    parser.add_argument('--max_target_length', type=int, default=-1, help='')
    ...
    if args.mode == 'data2text':
        ...
        cnndm_app = ' --max_source_length {} --train_max_target_length {} ' \
                   '--val_max_target_length {} --dataloader_num_workers 4'.format(max_source_length, max_target_length, val_max_target_length)
    ...
    elif args.mode == 'bioleaflets':
        TRAIN_FILE = "../data/bioleaflets/train.source"
        TEST_FILE = "../data/bioleaflets/val.source"

        max_source_length = 512 - 2 - args.preseqlen//2
        max_target_length = 512 - args.preseqlen//2
        val_max_target_length = max_target_length
        test_max_target_length = max_target_length

        bioleaflets_app = ' --max_source_length {} --train_max_target_length {} ' \
                   '--val_max_target_length {} --dataloader_num_workers 4 '.format(max_source_length, max_target_length,
                                                         val_max_target_length, )
        ...
        TRAIN_FILE = "../data/medparasimp/train.source"
        TEST_FILE = "../data/medparasimp/val.source"
        ...
        medparasimp_app = ' --max_source_length {} --train_max_target_length {} ' \
                   '--val_max_target_length {} --dataloader_num_workers 4 '.format(max_source_length, max_target_length,
                                                         val_max_target_length, )
    ...
    if args.dir_name is None:
        Model_FILE = args.mode + args.tuning_mode + '_' + args.model_nickname + '_' + args.optim_prefix[:1] + '_' + str(args.preseqlen) + \
    ...
    _instruct = f'_useTaskInstruction{args.use_task_instruction}' if args.use_task_instruction else ""
    Model_FILE += _instruct
    ...
    OLD_MODEL = args.model_name_or_path

    app = "--optim_prefix {} --preseqlen {} --prefix_mode {} --format_mode {} " \
          "--gradient_accumulation_steps {} --learning_rate {} --warmup_steps {} --weight_decay {} --seed {} " \
        ...
        format(args.optim_prefix, args.preseqlen, args.prefix_mode, args.format_mode,
               args.gradient_accumulation_steps, args.learning_rate, args.warmup_steps, args.weight_decay, args.seed,
    ...
    if args.notes is not None and 'lowdata' in args.notes:
        ...
        app += f'--evaluate_during_training --eval_steps {args.eval_steps} '
    ...
    COMMANDLINE="python -u run_language_modeling.py \
        --output_dir={} \
        --model_type=gpt2 \
        --model_name_or_path={} \
        --tokenizer_name={} \
        ...
        --train_embs no ".format(Model_FILE, OLD_MODEL, OLD_MODEL, args.bsz, args.bsz, args.save_steps, args.epoch, TRAIN_FILE, args.mode, TEST_FILE,
                                 args.tuning_mode, logging_dir, args.use_task_instruction)
    ...
    # COMMANDLINE += f" 2>&1 | tee {Model_FILE}.log.txt"
    COMMANDLINE += f" | tee {Model_FILE}.log.txt"
