...
## =>> Sphinx1
python -m torch.distributed.launch --nproc_per_node 8 --nnodes 2 --node_rank 0 --master_addr=sphinx1.stanford.edu train.py --config conf/gpt2-benchmark-config.yaml --nnodes 2 --nproc_per_node 8 --training_arguments.per_device_train_batch_size 8 --run_id 25-ddp-n=2-g=8-fp32-dbsz=8; pkill -f "train.py"; sleep 3

## =>> Sphinx 2
python -m torch.distributed.launch --nproc_per_node 8 --nnodes 2 --node_rank 1 --master_addr=sphinx1.stanford.edu train.py --config conf/gpt2-benchmark-config.yaml --nnodes 2 --nproc_per_node 8 --training_arguments.per_device_train_batch_size 8 --run_id 25-ddp-n=2-g=8-fp32-dbsz=8; pkill -f "train.py"; sleep 3
