...
def get_auto_clm_tokenizer(
    ...
    elif "gpt_neo" in model_id and model_configs:
        config = GPTNeoConfig.from_dict(model_configs)
    else:
    ...
    elif "gpt_neo" in model_id:
        overwatch.info(f"Initializing Custom GPT-Neo Model from Configuration: `{REGISTRY[model_id]}`...")
        model = GPTNeoForCausalLM.from_pretrained(initial_weights, config=config)
    ...
    if initial_weights is not None and not os.path.isdir(initial_weights):
