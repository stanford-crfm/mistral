...
An example json config file is available at ``conf/deepspeed/z2-small-conf.json``:

.. include:: ../../conf/deepspeed/z2-small-conf.json
   :literal:
...
The following command (run on machine1) will launch training across your cluster: ::
    ...
    deepspeed --num_gpus 8 --num_nodes 2 --master_addr machine1 train.py --config conf/gpt2-micro.yaml --nnodes 2 --nproc_per_node 8 --training_arguments.fp16 true --training_arguments.per_device_train_batch_size 4 --training_arguments.deepspeed conf/deepspeed/z2-small-conf.json --run_id gpt2-micro > gpt2-micro.out 2> gpt2-micro.err

This assumes that the appropriate hostfile is set up at ``/job/hostfile`` on ``machine1``.

You should see output similar to the following in ``gpt2-micro.out`` if training is running as expected: ::
