...
To launch a training run, use this command (found in ``scripts/run/single-node.sh``) ::
    ...
    CUDA_VISIBLE_DEVICES=0 python train.py --file conf/tutorial-gpt2-micro.yaml --nnodes 1 --nproc_per_node 1 --training_arguments.fp16 true --training_arguments.per_device_train_batch_size 2
